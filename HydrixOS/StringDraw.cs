using Cosmos.System.Graphics;
using System;
using System.Drawing;
using System.IO;
using System.Text;


namespace HydrixOS.GUI.String
{
    public static class ASC16
    {
        static string ASC16Base64 = "";
        static MemoryStream ASC16FontMS = new MemoryStream(Convert.FromBase64String(ASC16Base64));

        //Note: the size of font is 8*16

        public static void DrawString(this Canvas canvas, Pen color, string s, int x, int y)
        {
            //ASCII
            string[] lines = s.Split('\n');
            for (int l = 0; l < lines.Length; l++)
            {
                for (int c = 0; c < lines[l].Length; c++)
                {
                    int offset = (Encoding.ASCII.GetBytes(lines[l][c].ToString())[0] & 0xFF) * 16;
                    ASC16FontMS.Seek(offset, SeekOrigin.Begin);
                    byte[] fontbuf = new byte[16];
                    ASC16FontMS.Read(fontbuf, 0, fontbuf.Length);

                    for (int i = 0; i < 16; i++)
                    {
                        for (int j = 0; j < 8; j++)
                        {
                            if ((fontbuf[i] & (0x80 >> j)) != 0)
                            {
                                canvas.DrawPoint(color, (x + j) + (c * 8), y + i + (l * 16));
                            }
                        }
                    }
                }
            }
        }
        public static void DrawString(this Canvas canvas, Pen color, string s, System.Drawing.Point point)
        {
            //ASCII
            string[] lines = s.Split('\n');
            for (int l = 0; l < lines.Length; l++)
            {
                for (int c = 0; c < lines[l].Length; c++)
                {
                    int offset = (Encoding.ASCII.GetBytes(lines[l][c].ToString())[0] & 0xFF) * 16;
                    ASC16FontMS.Seek(offset, SeekOrigin.Begin);
                    byte[] fontbuf = new byte[16];
                    ASC16FontMS.Read(fontbuf, 0, fontbuf.Length);

                    for (int i = 0; i < 16; i++)
                    {
                        for (int j = 0; j < 8; j++)
                        {
                            if ((fontbuf[i] & (0x80 >> j)) != 0)
                            {
                                canvas.DrawPoint(color, (point.X + j) + (c * 8), point.Y + i + (l * 16));
                            }
                        }
                    }
                }
            }
        }
        public static void DrawString(this Canvas canvas, Pen color, string s, int x, int y, int fontSize)
        {
            // Calculate the scaling factor for the font size
            float scaleFactor = (float)fontSize / 16.0f;

            // ASCII
            string[] lines = s.Split('\n');
            for (int l = 0; l < lines.Length; l++)
            {
                for (int c = 0; c < lines[l].Length; c++)
                {
                    int offset = (Encoding.ASCII.GetBytes(lines[l][c].ToString())[0] & 0xFF) * 16;
                    ASC16FontMS.Seek(offset, SeekOrigin.Begin);
                    byte[] fontbuf = new byte[16];
                    ASC16FontMS.Read(fontbuf, 0, fontbuf.Length);

                    for (int i = 0; i < 16; i++)
                    {
                        for (int j = 0; j < 8; j++)
                        {
                            if ((fontbuf[i] & (0x80 >> j)) != 0)
                            {
                                // Scale the coordinates based on the fontSize
                                int scaledX = (int)(x + j * scaleFactor) + (c * 8);
                                int scaledY = (int)(y + i * scaleFactor) + (l * 16);

                                canvas.DrawPoint(color, scaledX, scaledY);
                            }
                        }
                    }
                }
            }
        }
        public static void DrawString(this Canvas canvas, Pen color, string s, System.Drawing.Point point, int fontSize)
        {
            // Calculate the scaling factor for the font size
            float scaleFactor = (float)fontSize / 16.0f;

            // ASCII
            string[] lines = s.Split('\n');
            for (int l = 0; l < lines.Length; l++)
            {
                for (int c = 0; c < lines[l].Length; c++)
                {
                    int offset = (Encoding.ASCII.GetBytes(lines[l][c].ToString())[0] & 0xFF) * 16;
                    ASC16FontMS.Seek(offset, SeekOrigin.Begin);
                    byte[] fontbuf = new byte[16];
                    ASC16FontMS.Read(fontbuf, 0, fontbuf.Length);

                    for (int i = 0; i < 16; i++)
                    {
                        for (int j = 0; j < 8; j++)
                        {
                            if ((fontbuf[i] & (0x80 >> j)) != 0)
                            {
                                // Scale the coordinates based on the fontSize
                                int scaledX = (int)(point.X + j * scaleFactor) + (c * 8);
                                int scaledY = (int)(point.Y + i * scaleFactor) + (l * 16);

                                canvas.DrawPoint(color, scaledX, scaledY);
                            }
                        }
                    }
                }
            }
        }
    }
}
